...
async function dirSync() {
    ...
    for (let k = dir.length - 1; k >= 0; k--) {
        ...
        if (folder.isDirectory()) {
            ...
            const { birthtime } = await fs_1.default.promises.stat(realPath);
    ...
    const newDB = lodash_1.default.sortBy(db, ['createdAt']);
    // console.log(newDB)
    return newDB;
}
exports.dirSync = dirSync;
async function mangaData(manga) {
...
async function updateCovers() {
    const dirs = await fs_1.default.promises.readdir(DJ_PATH);
    const coverdirs = [];
    const imagePromises = [];
    ...
    for (var i = 0; i < coverdirs.length; i++) {
        ...
        const image = await jimp_1.default.read(cover);
